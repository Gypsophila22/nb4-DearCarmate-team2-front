...
import { AxiosErrorData, CompanyUserType, OffsetPagination } from "@shared/types";
...
const useDeleteUser = () => {
  ...
  const mutation = useMutation<{ message: string }, AxiosError<AxiosErrorData>, number, unknown>({
    ...
    onSuccess: async () => {
      ...
      const queryData: OffsetPagination<CompanyUserType> | undefined = queryClient.getQueryData([
        "users",
    ...
    onError: (error) => {
      const text = error?.response?.data?.message || "유저 정보 삭제에 실패했습니다.";
      openConfirmModal({
        text,
      });
    },
