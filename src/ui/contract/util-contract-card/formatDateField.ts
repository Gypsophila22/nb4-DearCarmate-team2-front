...
const formatDateField = (
  ...
  resolutionDate: ContractType["resolutionDate"],
  meetings: ContractType["meetings"],
) => {
  ...
  switch (status) {
    ...
    default:
      ...
      if (meetings.length > 0) {
        ...
        const futureMeeting = sortedMeetings.find((meeting) =>
          isAfter(parseISO(meeting.date), now),
        );
        const meetingToFormat = futureMeeting
          ? futureMeeting
