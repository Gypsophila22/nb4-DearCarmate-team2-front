...
import { AxiosErrorData, ContractStatus, ContractType, ContractsListType } from "@shared/types";
  ...
  >({
    ...
    onSuccess: (newContract, { id, data, prevStatus }) => {
      ...
      queryClient.setQueryData(["contracts", { keyword, searchBy }], {
        ...
        [prevStatus]: {
          data: queryData[prevStatus].data.filter((contract) => contract.id !== id),
    ...
    onError: (error) => {
      const text = error?.response?.data?.message || "계약 건 상태 수정에 실패했습니다.";
      openConfirmModal({
        text,
      });
    },
